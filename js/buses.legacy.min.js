...
function convertLineName(line, options) {
  if (options.line_name_map && options.line_name_map[line]) return options.line_name_map[line];
...
function createTableHeader(allBuses) {
  ...
  thead.appendChild(fillTr(['车牌号'].concat(allBuses.map(function (bus) {
...
function convertBusQuery(queryInput) {
  ...
  conditions.forEach(function (condition) {
    ...
    } else if (match = busIdRegEx.exec(condition)) query.busId.push(match[1] + '-' + match[2]);else if (match = licenseIdRegEx.exec(condition)) query.licenseId.push(match[1].toUpperCase());
