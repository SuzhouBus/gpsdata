...
var LineDataManager = function () {
  ...
  LineDataManager.prototype.load = function () {
    var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(startDate, endDate) {
      ...
      var dataToLoad, startMonth, endMonth, archivedSources, _loop, currentMonth, _loop2, _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, item;
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              if (startDate < this.manifest.start_date) {
                ...
                if (startDate <= endDate) {
                  startMonth = DateUtils.toYearMonth(startDate);
                  endMonth = DateUtils.toYearMonth(endDate);
                  archivedSources = Object.keys(this.manifest.archives || {}).map(function (source) {
                  ...
                  _loop = function _loop(currentMonth) {
                  ...
                  for (currentMonth = DateUtils.toYearMonth(startDate); currentMonth <= DateUtils.toYearMonth(endDate); currentMonth = DateUtils.nextMonth(currentMonth)) {
                    _loop(currentMonth);
                  }
                }
              }
                    ...
                    switch (_context.prev = _context.next) {
                      case 0:
                        ...
                        if (item.month == 'current') {
                          path = _this3.manifest.data[item.source];
                        } else {
                          path = _this3.manifest.archives[item.source].path + item.month.replace('-', '') + '.json';
                        }
                        _context.next = 4;
                        return fetch(path).then(function (x) {
                          return x.json();
                        }).then(function (data) {
                          if (!_this3.loadedLineData_[item.month]) _this3.loadedLineData_[item.month] = {};
                          _this3.loadedLineData_[item.month][item.source] = data;
                          _this3.importData_(item.month, item.source);
                          item.loaded = true;
                          _this3.onUpdateProgress && _this3.onUpdateProgress(dataToLoad, item.month, item.source, 1, 1);
                        })['catch'](function (_) {
            ...
            case 9:
              ...
              item = _step.value;
              return _context2.delegateYield(_loop2(item), 't0', 12);
  ...
  LineDataManager.prototype.importData_ = function importData_(month, source) {
    ...
    Object.keys(data).forEach(function (line) {
      var lineName = _this4.lineNameMap_[source][line] || line;
      if (!_this4.lineData_[lineName]) _this4.lineData_[lineName] = {};
      _this4.lineData_[lineName][month] = data[line];
    });
  };

  LineDataManager.prototype.query = function query(lineOrLines, startDate, endDate) {
    var _this5 = this;
    ...
    lineOrLines.forEach(function (line, lineIndex) {
      Object.keys(_this5.lineData_[line]).filter(function (month) {
        if (month == 'current') {
          return _this5.isRangeOverlapped_(startDate, endDate, _this5.manifest.start_date, _this5.today_);
        } else {
          return DateUtils.toYearMonth(startDate) <= month && DateUtils.toYearMonth(endDate) >= month;
        }
      }).sort().forEach(function (month) {
        // Note: 'current' is always sorted after yyyy-mm.
        var currentLineData = _this5.lineData_[line][month];
    ...
    var buses = Object.keys(allBusesMap).filter(function (licenseId) {
      return allBusesMap[licenseId]['hasWeight'];
    }).sort(function (licenseA, licenseB) {
      var a = allBusesMap[licenseA];
      var b = allBusesMap[licenseB];
  ...
  LineDataManager.prototype.contains = function contains(lineOrLines) {
    var _this6 = this;

    if (typeof lineOrLines == 'string') lineOrLines = [lineOrLines];
    return !lineOrLines.some(function (line) {
      return !_this6.lineData_[line];
    });
  };

  LineDataManager.prototype.getLines = function getLines(naturalSort) {
    return Object.keys(this.lineData_).sort(function (a, b) {
      var ia = parseInt(a);
      var ib = parseInt(b);
      ...
      if (!naturalSort) return defaultComparisonResult;
...
var PALETTE = [[230, 25, 75], [60, 180, 75], [255, 225, 25], [0, 130, 200], [245, 130, 48], [145, 30, 180], [70, 240, 240], [240, 50, 230], [210, 245, 60], [250, 190, 190], [0, 128, 128], [170, 110, 40], [128, 0, 0], [170, 255, 195], [128, 128, 0], [0, 0, 128], [128, 128, 128]];
...
(function () {
  ...
  if (month == today.getMonth()) today.setDate(0);
...
function showLinesNew(lineOrLines) {
  ...
  var data = lineDataManager.query(lineOrLines, currentStartDate, currentEndDate);
...
function parseUrlHash() {
  if (location.hash.replace('#', '')) {
    ...
    } else showLinesNew(lineChooser.value = hashValue);
    return true;
  }
}
...
function init() {
  lineDataManager.load(currentStartDate, currentEndDate).then(function (_) {
    document.getElementById('progress').style.display = 'none';
    updateLineChooser(lineDataManager.getLines());
    if (!parseUrlHash()) showLinesNew(document.getElementById('lineChooser').children[0].value);
    loadRemoteManifest();
  });
    ...
    [].map.call(document.getElementById('resultList').children, function (option) {
      ...
      for (var i = Math.max(days.length - findRecentDays, 0); i < days.length; ++i) {
        ...
        if (weight) {
          ...
          if (results[date]) results[date].push(item);else results[date] = [item];
        }
      }
    });
