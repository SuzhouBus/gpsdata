...
var LineDataManager = function () {
  function LineDataManager(manifest, fetchSupportsReadableStream) {
  ...
  LineDataManager.prototype.importData_ = function importData_(month, source) {
    ...
    Object.keys(data).forEach(function (line) {
      ...
      lineName = _this5.formatInternalLineName_(lineName, groupDetails);
      if (!_this5.lineData_[lineName]) _this5.lineData_[lineName] = {};
      _this5.lineData_[lineName][month] = data[line];

      _this5.lineGroupMap_[lineName] = groupId;
      if (!_this5.linesByGroup_[groupId]) _this5.linesByGroup_[groupId] = new Set([lineName]);else _this5.linesByGroup_[groupId].add(lineName);
    });
  };
  ...
  LineDataManager.prototype.getMonthsByRange_ = function getMonthsByRange_(startDate, endDate) {
    ...
    return Object.keys(this.loadedLineData_).filter(function (month) {
      if (month == 'current') {
        return _this6.isRangeOverlapped_(startDate, endDate, _this6.manifest.start_date, _this6.latestDate);
      } else {
  ...
  LineDataManager.prototype.getLines = function getLines() {
    ...
    return this.getGroups().map(function (group) {
      var result = Object.assign({}, group, {
        rawLines: _this10.sortLines_(Array.from(_this10.linesByGroup_[group.id])),
        lines: []
      });
      var options = _this10.manifest.line_groups[group.id] || _this10.manifest;
      ...
      result.rawLines.forEach(function (line) {
        ...
        if (relatedLinesMap[line]) {
          ...
          result.lines.push({ idlist: relatedGroup, displayName: _this10.getLineDisplayName(relatedGroup[0]) });
          return;
        }
        ...
        if (lastMatch && match && lastMatch[1] == match[1] && lastMatch[3] == match[3] && !unrelatedLines.includes(match[1] + match[2])) {
          ...
          if (!lastLine.idlist) {
            ...
            var currentDisplayName = _this10.getLineDisplayName(line);
          ...
          result.lines.push({ id: line, displayName: _this10.getLineDisplayName(line) });
        }
        lastMatch = match;
      });
  ...
  LineDataManager.prototype.sortLines_ = function sortLines_(lines) {
    ...
    return lines.sort(function (a, b) {
      ...
      if (_this11.manifest.line_name_map) {
        if (_this11.manifest.line_name_map[a]) a = _this11.manifest.line_name_map[a];
        if (_this11.manifest.line_name_map[b]) b = _this11.manifest.line_name_map[b];
      }

      if (pureNumberRegEx.test(a) && pureNumberRegEx.test(b)) return _this11.compareNumbers_(a, b);
      ...
      var result = _this11.compareWithoutCopyNumber_(a, b);
      return result == 0 ? _this11.compareNumbers_(a.copyNumber || -1, b.copyNumber || -1, 'natural') : result;
    });
  };
  ...
  LineDataManager.prototype.sortBuses_ = function sortBuses_(buses) {
    ...
    return buses.sort(function (a, b) {
      if (a.busId && b.busId) {
        ...
        return 1;else return _this12.defaultCompare_(a.licenseId, b.licenseId); // Buses without busId are sorted by licenseId.
    });
  };
  ...
  LineDataManager.prototype.getGroups = function getGroups() {
    ...
    return Object.keys(this.linesByGroup_).sort(function (a, b) {
      ...
      if (indexA != -1 && indexB != -1) return indexA - indexB;else if (indexA == -1 && indexB != -1) return 1; // (remaining groups > defined groups)
        ...
        else return _this13.defaultCompare_(a, b);
    }).map(function (group) {
      return { id: group, displayName: (_this13.manifest.line_groups[group] || {}).name || group };
    });
  };
...
function loadManifest() {
  var fetchSupportsReadableStream = false;
...
function loadBusUpdates() {
  ...
  return fetch('newbuses.csv').then(function (r) {
    ...
    replaceChildren(updates_div, createElement('table', [createElement('thead', createElement('tr', ['时间', '线路', '自编号', '车牌号'].map(function (x) {
      ...
      var lineName = convertLineName(item.line, manifest);
