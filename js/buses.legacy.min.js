...
function loadBusUpdates() {
  ...
  return fetch('newbuses.csv', { cache: 'no-cache' }).then(function (r) {
    ...
    replaceChildren(updates_div, createElement('table', [createElement('thead', createElement('tr', ['时间', '线路', '自编号', '车牌号'].map(function (x) {
      ...
      return createElement('tr', [createElement('td', item.update_time.substring(5)), createElement('td', createElement('a', convertLineName(item.line, manifest), { href: '#' + lineDataManager.getLineFullName(item.line) })), createElement('td', (lineDataManager.queryCurrentBus(item.line, item.licenseId) || {}).busId || ''), createElement('td', item.licenseId)]);
    }))]));
  });
}
